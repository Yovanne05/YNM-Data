...
def create_titre(titre: Titre) -> None:
    con = db.get_db_connection()
    print("Le titre là ptn")
    try:
        if not check_coherences_dates(titre.id_date_debut_licence, titre.id_date_fin_licence):
...
def check_coherences_dates(id_date_debut_licence: int, id_date_fin_licence: int) -> bool or None:
    ...
    try:
        with con.cursor() as cursor:
            cursor.execute("SELECT * FROM temps WHERE idDate=%s", (id_date_debut_licence,))
            result_debut = cursor.fetchall()
            cursor.execute("SELECT * FROM temps WHERE idDate=%s", (id_date_fin_licence,))
            ...
            print("temps_debut", temps_debut.jour, temps_debut.mois, temps_debut.annee)
            print("temps_fin", temps_fin.jour, temps_fin.mois, temps_fin.annee)
